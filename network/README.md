...
## 章节概览
本章节介绍 Ansible 中网络配置与监控的核心模块，涵盖防火墙规则管理（firewalld/ufw/iptables）与网络连接监控（wait_for）。这些模块广泛应用于安全加固、容器编排、多层部署等场景，所有示例均包含中文注释和最佳实践说明。

## 与 system/firewall 的区别
本 `network/` 目录专注于**跨系统网络配置与监控**的通用模块：
- **network/ 职责**：防火墙规则管理、端口监控、网络配置下发（不涉及系统初始化）
